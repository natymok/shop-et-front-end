...
const DropdownUser = () => {
  const [{token,TotalSell,amount,customer,companyName,transaction},dispatch]=useStateValue()
  const downloadReport=()=>{
    const reportdata=`companyName==${companyName}
Total sell=${TotalSell},
customer=${customer}
total profit from january-december
january==${transaction[0]}
february==${transaction[1]}
march==${transaction[2]}
april==${transaction[3]}
may==${transaction[4]}
june==${transaction[5]}
july==${transaction[6]}
august==${transaction[7]}
september==${transaction[8]}
october==${transaction[9]}
November==${transaction[10]}
december==${transaction[11]}`
const doc=new jsPDF()
doc.text(reportdata,10,10)
doc.save("report.pdf")
   /* const filenamne="report.pdf"
    const fileurl=URL.createObjectURL(new Blob([doc],{type:'application/pdf'}))
    const link=document.createElement('a')
    link.href=fileurl
    link.setAttribute('download',filenamne)
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <button onClick={downloadReport} className="flex items-center gap-3.5 py-4 px-6 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
