...
const ECommerce = () => {
  let topBuy = [];
  const [{ token, TotalSell, amount, customer, transaction }, dispatch] =
    useStateValue();
  ...
  useEffect(() => {
    ...
    axiosinstance
      .get('/getCompany')
      .then((res) => {
        _amount = res.data.message.filter(
    ...
    if (tokken) {
      ...
      axiosinstance
        .post('/getTotalstock', { companyName: CompanyName })
        .then((res) => {
      ...
      axiosinstance
        .post('/topbuy', { companyName: CompanyName })
        .then((res) => {
          if (res.status == '200') {
            dispatch({
              type: 'topbuy',
